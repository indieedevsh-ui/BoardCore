...
struct BossFightARView: View {
    ...
    var body: some View {
        ...
        .onDisappear {
            roundTask?.cancel()
            trikiCoordinator.setLongPressHoldDuration(1.5)
        }
        .onAppear {
            trikiCoordinator.setLongPressHoldDuration(0.8)
        }
    ...
    private func handleTrikiMotionGestureFromCoordinator() {
        ...
        let action: TrikiGestureAction = switch kind {
        case .rotateLeft: rules.rotateLeft
        case .rotateRight: rules.rotateRight
        case .shake: rules.shake
        case .slide: rules.slide
        case .click, .physicalButton: .none
        }
        switch action {
        case .listDown:
        ...
        case .attack:
            playerSelectedMove(.attack)
        case .defense:
            playerSelectedMove(.defense)
        case .strongAttack:
            playerSelectedMove(.strongAttack)
        default:
            break
        }
    }
...
// MARK: - AR / camera scanner

private final class BossFightCameraContainerView: UIView {
...
private struct BossFightARScanner: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> BossFightCameraContainerView {
        let container = BossFightCameraContainerView(frame: .zero)
        context.coordinator.attach(to: container)
        context.coordinator.updateCameraPosition(cameraPosition)
        return container
    }

    func updateUIView(_ uiView: BossFightCameraContainerView, context: Context) {
        context.coordinator.onPayloads = onPayloads
        context.coordinator.updateCameraPosition(cameraPosition)
        context.coordinator.layoutPreview(in: uiView)
    }

    static func dismantleUIView(_ uiView: BossFightCameraContainerView, coordinator: Coordinator) {
    ...
    final class Coordinator: NSObject, ARSessionDelegate, AVCaptureVideoDataOutputSampleBufferDelegate {
        ...
        private let visionQueue = DispatchQueue(label: "dmd.bossfight.ar", qos: .userInitiated)
        private let captureQueue = DispatchQueue(label: "dmd.bossfight.capture", qos: .userInitiated)
        ...
        private weak var containerView: BossFightCameraContainerView?
        ...
        func attach(to container: BossFightCameraContainerView) {
        ...
        func layoutPreview(in container: BossFightCameraContainerView) {
        ...
        private func startCaptureSession(in container: BossFightCameraContainerView) {
            captureQueue.async { [weak self] in
                guard let self else { return }
